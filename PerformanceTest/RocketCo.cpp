...
class coroutine_context {
public:
    coroutine_context(RocketCo::Co_Attribute* libco_attr, int share_stack) {
        callee_ctx_ = NULL;
        share_stack_ = share_stack;
        // 注意要执行的参数为成员函数中的start_callback
        RocketCo::Co_Create(&callee_ctx_, libco_attr, &start_callback, this);
    ...
    ~coroutine_context() {
        if (NULL != callee_ctx_) {
            RocketCo::FreeCo_Entity(callee_ctx_);
        }
    }
    ...
    void resume() {
        ...
        RocketCo::Co_resume(callee_ctx_);
    ...
    void yield() {
        ...
        // 把CPU执行权切换到调用栈的上一层
        RocketCo::Co_yeild(callee_ctx_->env);
...
private:
    RocketCo::Co_Entity* callee_ctx_;
    int share_stack_;
...
int main(int argc, char *argv[]) {
    ...
    if (0 != enable_share_stack) {
        libco_attr.shareStack = new RocketCo::Co_ShareStack(1, libco_attr.stack_size);
        //libco_attr.shareStack = co_alloc_sharestack(1, libco_attr.stack_size);
    } else {
        // 协程数量个共享栈，最大程度减少协程切换时的开销
        libco_attr.shareStack = new RocketCo::Co_ShareStack(max_coroutine_number, libco_attr.stack_size);
        //libco_attr.shareStack = co_alloc_sharestack(max_coroutine_number, libco_attr.stack_size);
    }
